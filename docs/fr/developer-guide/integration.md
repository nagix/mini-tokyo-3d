...
Une variété de [plugins](../user-guide/plugins.md) sont disponibles pour afficher des informations supplémentaires sur la carte 3D. Les plugins sont fournis séparément de Mini Tokyo 3D et peuvent être installés au moment de l'installation du site ou de la création de l'application, selon vos préférences. À titre d'exemple, ce qui suit montre comment incorporer [Precipitation Plugin](https://github.com/nagix/mt3d-plugin-precipitation) et [Fireworks Plugin](https://github.com/nagix/mt3d-plugin-fireworks).
